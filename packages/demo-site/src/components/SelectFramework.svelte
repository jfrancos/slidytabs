...
import { type Framework, frameworks, icons } from "@/lib/utils";
import { List, Root, Trigger } from "@/shadcn-svelte/tabs";
...
>
  <List class="*:px-2 *:gap-1 sm:*:px-4 sm:*:gap-2 *:h-7 rounded-md text-sm">
    {#each frameworks as framework}
      <Trigger value={framework} data-state={framework === "react" ? "active" : "inactive"}>
